  ...
  try {
    ...
    try {
      ...
      if (connError.code === "ECONNREFUSED") {
        ...
        console.error(
          "   Please check if MySQL server is running and accessible."
        );
      } else if (connError.code === "ER_ACCESS_DENIED_ERROR") {
        console.error("\n🔴 Access denied for the specified MySQL user.");
        console.error(
          "   Please verify the username and password in your .env file."
        );
      } else if (connError.code === "ER_BAD_DB_ERROR") {
        console.error(`\n🔴 Database '${dbConfig.database}' does not exist.`);
        console.error(
          "   Please create the database or check the database name in your .env file."
        );
      }
    ...
    try {
      const [dbs] = await connection.query("SHOW DATABASES LIKE ?", [
      ...
      if (dbs.length === 0) {
        ...
        console.error(
          "   Please create the database using: CREATE DATABASE " +
            (process.env.DB_NAME || "MeditActive") +
            ";"
        );
        process.exit(1);
      ...
      console.log(
        `✅ Database '${process.env.DB_NAME || "MeditActive"}' exists`
      );
      ...
      const [[{ user_count }]] = await connection.query(
