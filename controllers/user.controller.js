...
// Base validation rules for user input (used for both create and update)
  ...
  body('obiettivo')
    ...
    .custom((value, { req }) => {
      const validGoals = [
        ...
        'massa muscolare', 'muscle gain'
      ];
      
      if (!validGoals.includes(value)) {
        throw new Error('Invalid goal type. Valid values are: "Perdita di Peso" or "Weight Loss", "Mantenimento" or "Maintenance", "Massa Muscolare" or "Muscle Gain"');
      }
      
      // Map to consistent values for storage
      const goalMap = {
      ...
      // Update the request body with the standardized value
...
// Validation for create operations (POST)
export const validateUserInput = [
  // Make required fields for POST
  body('firstName')
...
// Validation for update operations (PATCH)
export const validateUserUpdate = [
  // Include base validation rules (all fields optional)
  ...baseUserValidationRules,
...
export const getAllUsers = async (req, res) => {
  ...
  try {
    ...
    const limit = Math.min(parseInt(req.query.limit, 10) || 10, 100); // Cap at 100 items per page
    const offset = Math.max(0, parseInt(req.query.offset, 10) || 0); // Ensure offset is not negative
    
    console.log(`[${requestId}] Fetching users with pagination:`, { limit, offset });
    ...
    try {
      ...
      console.log(`[${requestId}] ✅ Successfully retrieved ${result.data.length} users from database`);
      ...
      console.error(`[${requestId}] ❌ Database error in User.findAll():`, errorDetails);
...
export const getUserById = async (req, res) => {
  try {
    ...
    const statusCode = error.code === 'ER_PARSE_ERROR' || 
                      error.code === 'ER_BAD_FIELD_ERROR' ||
                      error.sqlMessage ? 400 : 500;
...
export const createUser = async (req, res) => {
  ...
  try {
    ...
    const { firstName, lastName, email, obiettivo, dataInizio, dataFine } = req.body;
    ...
    try {
      ...
      // Make sure we're sending the response with proper headers and body
      const response = {
        status: 'success',
        data: user,
        requestId
      ...
      // Explicitly set status and send JSON
      res.status(201);
    ...
    console.error(`[${requestId}] ❌ Error in createUser (${errorId}):`, errorDetails);
      ...
      switch (error.code) {
        ...
        case 'ER_NO_SUCH_TABLE':
          return res.status(500).json({
            ...
            details: 'The users table is missing. Please run the database migrations.',
            errorId,
            requestId
          });
...
export const updateUser = async (req, res) => {
  ...
  try {
    // ID is already validated by the route middleware
    const { id } = req.params;
    ...
    // Date validations
    if (dataInizio !== undefined && dataInizio && isNaN(Date.parse(dataInizio))) {
      const error = new Error('Invalid start date format. Use ISO format (YYYY-MM-DD)');
      error.code = 'INVALID_START_DATE';
      throw error;
    }

    if (dataFine !== undefined && dataFine && isNaN(Date.parse(dataFine))) {
      const error = new Error('Invalid end date format. Use ISO format (YYYY-MM-DD)');
